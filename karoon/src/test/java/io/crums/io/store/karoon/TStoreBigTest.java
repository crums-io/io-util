 ...
 * @author Babak
 */
public class TStoreBigTest extends IoTestCase {
  ...
  void initUnitTestDir(Object methodLabel)  {
    this.testDir = getMethodOutputFilepath(methodLabel);
  ...
  public void test64ByteRow() throws Exception {
    ...
    initUnitTestDir(label);
    if (!checkEnabled(label))
      return;
    
    log.info("+----------------------------------------------");
    log.info("|");
    log.info("| " + method(label) + " write-heavy stress test");
    log.info("|");
    log.info("+----------------------------------------------");
...
//    row.flip();
  ...
  private boolean checkEnabled(Object label) {
    boolean enabled =
        "true".equalsIgnoreCase(System.getProperty(RUN_PROPERTY)) ||
        Boolean.valueOf(System.getProperty(PERF_TEST_PROPERTY)) ||
        getClass().getSimpleName().equals(System.getProperty("test"));
    if (!enabled) {
      log.info(
          "Skipping " + method(label) +
          ". To activate -D" + RUN_PROPERTY + "=true or -D" +
          PERF_TEST_PROPERTY + "=true or -Dtest=" +
