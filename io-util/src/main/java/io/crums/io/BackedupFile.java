...
/**
 * Utility for backing up a file before creating a new version of it. By default uses the tilde ('~') to prefix the backup file.
 * To make it hidden, supply a period ('.') in the constructor.
 * 
 * <h2>2-File Model</h2>
 * <p>
 * This doesn't actually have a commit method: it has a {@linkplain #rollback()} method.
...
public class BackedupFile {
   ...
   */
  public BackedupFile(File file, char tilde) throws IllegalArgumentException {
    this.tilde = tilde;
   ...
   * @return {@code this}
   */
  public final BackedupFile moveToBackup() throws IllegalStateException {
    if (file.exists()) {
      File backup = getBackupFile();
      if (backup.exists()) {
        if (!backup.delete())
